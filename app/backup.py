...
with st.sidebar:
    ...
    # Classification method with icons
...
if 'full_df' not in st.session_state:
    st.session_state['full_df'] = pd.read_csv("data/reduced_lyon_buildings.csv")
...
with st.sidebar:
    ...
    df['class_label'] = df[class_column_mapping[classification_method]]
...
with st.expander("🔍 Advanced Filters", expanded=False):
    co2_min, co2_max = st.slider("CO₂ Usage (kg)", float(df.CO2_Usage.min()), float(df.CO2_Usage.max()), (float(df.CO2_Usage.min()), float(df.CO2_Usage.max())))
    water_min, water_max = st.slider("Water Usage (L)", float(df.Water_Usage.min()), float(df.Water_Usage.max()), (float(df.Water_Usage.min()), float(df.Water_Usage.max())))
    energy_min, energy_max = st.slider("Energy (kWh)", float(df.Energy_Consumption.min()), float(df.Energy_Consumption.max()), (float(df.Energy_Consumption.min()), float(df.Energy_Consumption.max())))
    
    st.subheader("Building Class Filter")
    all_classes = ['A', 'B', 'C', 'D', 'E', 'F']
    class_colors = {'A': [40,167,69,180], 'B': [92,184,92,180], 'C': [255,193,7,180], 'D': [253,126,20,180], 'E': [220,37,69,180], 'F': [108,117,125,180]}
    selected_classes = []
    cols = st.columns(len(all_classes))
    for i, cls in enumerate(all_classes):
        with cols[i]:
            if st.checkbox(f"Class {cls}", True, key=f"class_{cls}"):
                selected_classes.append(cls)

    color_by = st.selectbox(
        "Color Buildings By",
        ["class_label", "CO2_Usage", "Water_Usage", "Energy_Consumption"],
        format_func=lambda x: {"class_label":"Energy Class","CO2_Usage":"CO₂ Emissions","Water_Usage":"Water Consumption","Energy_Consumption":"Energy Usage"}[x]
    )

filtered_df = df[
    (df.CO2_Usage.between(co2_min, co2_max)) &
    (df.Water_Usage.between(water_min, water_max)) &
    (df.Energy_Consumption.between(energy_min, energy_max)) &
    (df.class_label.isin(selected_classes))
]

# Info bar
col1, col2, col3 = st.columns([2,2,1])
col1.info(f"Showing {len(filtered_df)} of {len(df)} buildings in {selected_city}")
col2.metric("Average Energy Class", filtered_df['class_label'].mode()[0] if not filtered_df.empty else 'N/A')
col3.metric("Data Last Updated", datetime.now().strftime("%Y-%m-%d"))

# Tabs
tab1, tab2, tab3, tab4 = st.tabs(["Interactive Map","Analytics & Insights","Building Data","Export & Reports"])

# Tab1: Progressive PyDeck drip loading
with tab1:
    st.markdown(f"<h2 style='text-align:center;'>Interactive Map: {selected_city}</h2>", unsafe_allow_html=True)
    map_placeholder = st.empty()

    # Initial deck with no layers
    ...
    for _, row in filtered_df.iterrows():
        color = class_colors[row['class_label']]
        layer = pdk.Layer(
            'ScatterplotLayer',
...
# Tab2: Analytics & Insights
with tab2:
    st.markdown("### Building Class Distribution")
    if not filtered_df.empty:
        display_distribution_plot(filtered_df, color_by)
    # ... other analytics tabs as before
